...
class AutoShutdownApp:
    def __init__(self, root=None):
        ...
        self.tray_icon = None
        self.current_mode = "timed"  # 当前模式：timed 或 countdown
    ...
    def setup_gui(self):
        ...
        self.root.geometry("500x700")  # 减小高度以适应新布局
    ...
    def create_widgets(self):
        ...
        # 滑动卡片容器
        self.card_container = ctk.CTkFrame(main_frame, fg_color="transparent")
        self.card_container.pack(fill="x", padx=20, pady=(0, 15))
        
        # 创建两个卡片
        self.create_timed_card()
        self.create_countdown_card()
        ...
        # 绑定左右滑动事件
        self.root.bind("<Left>", self.switch_to_timed)
        self.root.bind("<Right>", self.switch_to_countdown)
        
    def create_timed_card(self):
        """创建定时关机设置卡片"""
        self.timed_card = ctk.CTkFrame(self.card_container, corner_radius=12)
        ...
        # 时间输入区域
        input_frame = ctk.CTkFrame(self.timed_card, fg_color="transparent")
    ...
    def create_countdown_card(self):
        """创建倒计时设置卡片"""
        self.countdown_card = ctk.CTkFrame(self.card_container, corner_radius=12)
        ...
        # 倒计时输入区域
        input_frame = ctk.CTkFrame(self.countdown_card, fg_color="transparent")
        input_frame.pack(fill="x", padx=20, pady=(0, 15))
    ...
    def show_current_card(self):
        ...
        if self.current_mode == "timed":
            self.timed_card.pack(fill="x")
            self.mode_label.configure(text="🕐 定时关机模式 (← → 切换)")
        else:
            self.countdown_card.pack(fill="x")
            self.mode_label.configure(text="⏱️ 倒计时模式 (← → 切换)")
    
    def switch_to_timed(self, event=None):
        """切换到定时关机模式"""
        if self.current_mode != "timed" and not self.is_running:
            self.current_mode = "timed"
            self.show_current_card()
    
    def switch_to_countdown(self, event=None):
        """切换到倒计时模式"""
        if self.current_mode != "countdown" and not self.is_running:
